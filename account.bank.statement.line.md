...
**Ordenação padrão:** `internal_index desc`
...
- `amount_residual` **(float)** — Residual Amount 🔒 readonly
- `internal_index` **(char)** — Internal Reference 🔒 readonly
- `is_reconciled` **(boolean)** — Is Reconciled 🔒 readonly
- `transaction_details` **(json)** — Transaction Details 🔒 readonly
- `create_date` **(datetime)** — Created on 🔒 readonly
- `write_date` **(datetime)** — Last Updated on 🔒 readonly
- `cron_last_check` **(datetime)** — Cron Last Check
- `unique_import_id` **(char)** — Import ID 🔒 readonly
...
- `statement_id` **(many2one)** — Statement → `account.bank.statement`
- `payment_ids` **(many2many)** — Auto-generated Payments → `account.payment`
- `partner_id` **(many2one)** — Partner → `res.partner`
- `currency_id` **(many2one)** — Journal Currency 🔒 readonly → `res.currency`
- `foreign_currency_id` **(many2one)** — Foreign Currency → `res.currency`
  > The optional other currency if it is a multi-currency entry.
- `bank_statement_attachment_ids` **(one2many)** — Bank Statement Attachment 🔒 readonly → `ir.attachment`
- `attachment_ids` **(one2many)** — Attachments 🔒 readonly → `ir.attachment`
- `online_account_id` **(many2one)** — Online Account 🔒 readonly → `account.online.account`
- `online_link_id` **(many2one)** — Account Online Link 🔒 readonly → `account.online.link`
...
- `running_balance` **(monetary)** — Running Balance 🔒 readonly
- `country_code` **(char)** — Country Code 🔒 readonly
  > The ISO country code in two chars.  You can use this field for quick search.
- `statement_complete` **(boolean)** — Is Complete 🔒 readonly
- `statement_valid` **(boolean)** — Is Valid 🔒 readonly
- `statement_balance_end_real` **(monetary)** — Ending Balance 🔒 readonly
- `statement_name` **(char)** — Statement Name 🔒 readonly
